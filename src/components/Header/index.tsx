...
import { selectUser, logout, selectUserDetail } from 'features/user/userSlice';
...
function Header(props: WithWidth) {
	...
	const renderMenuUser = (
		...
		>
			...
			{userDetail.accountType === 'admin' && userDetail.length !== 0 ? (
				...
				<Autocomplete
							...
							>
								<Grid container>
									...
									<Grid spacing={4} style={{ padding: '4px' }}>
										...
										<Typography>{prod.prodBrand}</Typography>
									</Grid>
								</Grid>
							</Link>
						);
					...
					renderInput={(params) => (
						<TextField
							...
							InputLabelProps={{ classes: classes.searchField }}
						/>
					)}
				/>
			</div>
